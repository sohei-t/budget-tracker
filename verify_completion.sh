...
if [ $FAIL_COUNT -eq 0 ]; then
    ...
    if [ ! -f "about.html" ] || [ ! -f "audio_script.txt" ] || [ ! -f "generate_audio_gcp.js" ]; then
        ...
        echo -e "   ${YELLOW}python3 $AGENT_TEMPLATE_DIR/src/documenter_agent.py${NC}"
    fi

    if [ ! -f "explanation.mp3" ] && [ -f "generate_audio_gcp.js" ]; then
        echo -e "\n2. 音声を生成:"
        echo -e "   ${YELLOW}export GOOGLE_APPLICATION_CREDENTIALS=\"/path/to/your/gcp-credentials.json\"${NC}"
    ...
    if [ ! -f "launch_app.command" ]; then
        echo -e "\n3. 起動スクリプトを生成:"
        echo -e "   ${YELLOW}python3 $AGENT_TEMPLATE_DIR/src/launcher_generator.py${NC}"
    fi
fi
