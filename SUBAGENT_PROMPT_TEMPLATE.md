...
0. 画像アセット生成（TECH_STACK.mdで画像生成が必要な場合のみ）：
   ...
   2. GCP認証の自動セットアップ（CLAUDE.md参照）
      - 認証ファイル確認: $GOOGLE_APPLICATION_CREDENTIALS
...
2. モバイルゲーム判定（条件付き実行）：
   ...
   1. GYRO_CONTROLS_STANDARD.md を読み込む
      パス: $AGENT_TEMPLATE_DIR/GYRO_CONTROLS_STANDARD.md
...
3. **解説コンテンツ作成**（Webアプリ/ゲームの場合）：
   ...
   - 音声生成手順:
     ...
     2. 認証設定:
        ...
        - GCP TTS: $GOOGLE_APPLICATION_CREDENTIALS
...
```bash
python3 $AGENT_TEMPLATE_DIR/src/portfolio_publisher.py \
