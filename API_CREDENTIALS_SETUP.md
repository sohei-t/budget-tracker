...
# → credentials/gcp-credentials.json (or set GOOGLE_APPLICATION_CREDENTIALS env var)
...
```bash
# プロジェクトごとに認証ファイルを作成
$AGENT_TEMPLATE_DIR/credentials/
├── gcp-credentials.json               # デフォルト
├── project-a-key.json            # プロジェクトA用
└── project-b-key.json            # プロジェクトB用

# .env でプロジェクトを切り替え
GOOGLE_APPLICATION_CREDENTIALS=$AGENT_TEMPLATE_DIR/credentials/project-a-key.json
